...
export default function CloudflareSettings() {
  ...
  const loadSettings = async () => {
    try {
      const response = await api.cloudflare.getSettings()
  ...
  const handleSave = async (e) => {
    ...
    try {
      await api.cloudflare.saveSettings({ apiToken, accountId: accountId || null })
      setAlert({
        variant: 'default',
        title: 'Sucesso!',
  ...
  const handleDelete = async () => {
    try {
      await api.cloudflare.deleteSettings()
      setAlert({
        variant: 'default',
        title: 'Removido',
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Integração Cloudflare</h1>
        <p className="text-muted-foreground mt-2">
          Configure sua API token do Cloudflare para auto-configuração de domínios
        </p>
      </div>
